...
import bot.utility.UnicodeParser;
...
public class Weather
{
    ...
    public boolean makeChart(String city)
    {
        ...
        if (cityJson != null)
        {
            ...
            Pattern pattern = Pattern.compile("0[06]|1[28]h$");

            for (int i = 0; i < tempList.size() / 3 * 2 - 2; i++)
            {
            ...
            for (String date : dateList)
            {
                ...
                String lazyShortDate = date.substring(5, 13) + "h";
                Matcher matcher = pattern.matcher(lazyShortDate);
                if (!matcher.find())
                {
                    XAXIS.setTickLabelPaint(lazyShortDate, Color.darkGray);
                }
            }
